...
M3.5 评论私信基础层：见 [`docs/M3.5-comment-private-message-foundation.md`](docs/M3.5-comment-private-message-foundation.md)。新增评论私信任务类型、搜索主页/当前主页两种入口、`|` 分隔的评论匹配词、视频与用户上限、评论阶段/进度模型及悬浮窗通用进度契约；P3.5-B 已接入可拖动任务进度悬浮窗和权限引导，任务开始/暂停/结束会自动显示或移除；P4-B 已接入独立运行时，按节点优先打开第一个视频和评论区、读取评论候选、分页去重并识别“暂时没有更多了”；P4-C 已接入评论作者名称点击、主页语义私信入口确认和空格安全探测，P4-D 已接入评论区结束后的多视频切换与任务级用户去重，仍待有界真机闭环验收。旧 B 端私信任务默认行为和本地历史数据保持不变。

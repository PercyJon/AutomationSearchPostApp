...
- 可选全局动作间隔：设置页默认留空、不施加额外节流；操作员可保存 1000–5000ms，使点击、输入、返回和滑动共享最小间隔，下一次动作即生效
...
- 图像识别扩展点：除评论入口 P0 的受限 Alpha 模板回退外，`ImageMatcher` 仍默认使用 `NoOpImageMatcher`。该回退只在既有下一视频私有截图的右侧比例区域识别评论气泡，必须通过三孔位相对对比、两帧稳定、视频页确认和评论面板后验；不接入通用 OpenCV、不会用于风控/验证码/登录页。多分辨率真机回归完成前不宣称通用模板匹配已发布
...
M3-UI3.1 总览与待办视觉精修：总览指标改为紧凑的“彩色加粗数字 + 常规说明文字”；任务进展保留标题，使用更细的圆环并将非粗体百分比置于圆环中心；待办任务行统一为首页今日待办的浅蓝底、蓝色竖线和列表图标风格，同时保留多选与批量执行能力。

M3-UI3.2 待办列表简化：待办页移除顶部标题/提示语和“可多选”提示，保留“待办任务”作为内容标题；移除外层列表边框和底色；任务行移除右侧列表图标，改以多选框作为操作入口，首页“今日待办”同步采用多选框样式。

M3-UI3.3 待办与总览微调：待办任务复选框移至每行右侧；批量执行按钮改为页面末尾的无图标“开始任务”；总览两侧标题统一顶部对齐、字体和间距，任务进展圆环缩小至与左侧两行指标相近的高度。

M3-UI3.4 待办执行栏固定：待办列表改为独立可滚动区域， “开始任务”按钮固定在底部导航上方；选择任务后按钮即时启用，未选择时保持禁用，批量执行逻辑不变。

M3.5 评论私信基础层：见 [`docs/M3.5-comment-private-message-foundation.md`](docs/M3.5-comment-private-message-foundation.md)。新增评论私信任务类型、搜索主页/当前主页两种入口、`|` 分隔的评论匹配词、视频与用户上限、评论阶段/进度模型及悬浮窗通用进度契约；P3.5-B 已接入可拖动任务进度悬浮窗和权限引导，任务开始/暂停/结束会自动显示或移除；P4-B 已接入独立运行时，按节点优先确认作品页后打开第一个视频和评论区、读取评论候选、分页去重并识别“暂时没有更多了”；P4-C 已接入“头像 → 用户主页 → 纸飞机私信入口 → 空格安全探测 → 返回评论区”的闭环。评论候选只接受“左侧圆形头像 + 灰色名称 + 黑色评论”的结构，忽略定位、评论数、点赞/踩和其他控件描述；点击前会再次验证活头像节点的语义、几何和行位置，重绘后若路径落到评论文字、回复或其他控件会拒绝点击；P4-D 已接入评论区结束后的多视频切换与任务级用户去重。没有作品或私密账号会跳过当前用户；可选的“跳过置顶视频”会过滤带“置顶”标记的卡片。仍待有界真机闭环验收。旧 B 端私信任务默认行为和本地历史数据保持不变。

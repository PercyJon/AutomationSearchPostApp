...
M3.5 评论私信基础层：见 [`docs/M3.5-comment-private-message-foundation.md`](docs/M3.5-comment-private-message-foundation.md)。新增评论私信任务类型、搜索主页/当前主页两种入口、`|` 分隔的评论匹配词、视频与用户上限、评论阶段/进度模型及悬浮窗通用进度契约；旧 B 端私信任务默认行为和本地历史数据保持不变，评论流程尚未接入现有导航控制器。

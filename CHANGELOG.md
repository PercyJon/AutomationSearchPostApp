...
- 现象：走公网隧道下 75MB APK 只有约几十 KB/s，完整安装测不完。
- 预期：真机经 USB `adb reverse` 访问电脑 `127.0.0.1:8001` 完成本地下载。
- 实际：公网 HTTP 仍拒绝；仅 `http://127.0.0.1` / `localhost` 可明文。

### 最小修改

- `LoginEndpointPolicy` 允许回环 HTTP；注入回环地址时沿用已保存的 license，不改记住的 HTTPS 源站。
- 仅对 `127.0.0.1` / `localhost` 开放 cleartext。
- 下载进度在缺少 Content-Length 时用检测接口下发的 `fileSize`。

### 验证

- 单测：公网 `http://` 仍不可用；`http://127.0.0.1:8001` 可用。
- 真机 V2217A：检测走本机；下载 200 来自 `127.0.0.1`；装完 `versionCode=15` / `0.3.5-update-e2e`。未启动抖音。

---

## [未发布] 2026-08-28 —— App 自动检测更新与安装

### 验证前记录

- 需求：登录后每个进程自动检测一次；我的页可手动检测；有新版本弹窗，立即更新在弹窗内显示进度，完成后进入系统安装；后台可标记强制更新；任务运行中不强制更新。

### 最小修改

- 后端：`automation_app_update` 表、管理端上传/发布/停用、移动端检测与鉴权下载。
- 管理端：自动化管理 → App更新推送。
- App：登录后检测一次；我的 → 检测更新；SHA-256 校验后 FileProvider 安装。任务进行中不锁强制弹窗。

### 验证

- 后端 pytest：`test_api_module_automation_app_update.py` 3 passed。
- Android：`AppUpdatePolicyTest`；无新增 px / 点击坐标。

### 下一步

- 真机用低 versionCode 包拉已发布 APK，走完进度并进入系统安装页。不启动抖音。

---

## [未发布] 2026-08-28 —— 点击等待不再把搜索框/私信「回复」当成 IM 横幅

### 验证前记录

- 现象：输入关键词后要空等才提交搜索；点进第一个用户变慢；进私信后长时间不发，BACK 回列表，该用户记为发送失败。
- 复现记录：任务 `3992185b`「红木沙发-20260828-150529」，用户「祥蕴阁红木家具」`MESSAGE_SEND_FAILED`，原因 `Could not place the blank probe in the private-message input`，耗时约 38s；同任务后续用户约 4–5s 成功。
- 预期：关键词提交、点用户、私信输入不应被首页 IM 横幅等待拖住。
- 根因假设：`waitForTargetWindow` 用了 `find()`（直播通知 **或** IM 横幅）。搜索 EditText 宽短可点、私信页顶部「回复」都被当成横幅；每次动作空等最多 10.5s，超时后 `withLiveNode` 直接失败，三次输入约 38s 后跳过。

### 最小修改

- 点击等待只认直播通知（`findLiveNotification`）。IM 横幅仍只在启动/开搜用 `findWaitOnly`。
- IM 检测：排除可编辑搜索框；「回复」必须落在盖住搜索区的宽短横幅上，聊天线程里的回复不算。

### 验证

- 单测：搜索框、私信「回复」不再是 IM；首页带「回复」的横幅仍是。
- 无新增 px / 点击坐标。

### 下一步

- 真机跑一条 B 端私信：关键词提交、点第一个用户、空消息探测应恢复原来的节奏。

---

## [未发布] 2026-08-28 —— 去掉开搜前无横幅也跑的顶栏 OCR

### 验证前记录

- 现象：正常任务从首页到点右上角搜索变慢。
- 根因假设：点搜索前无论有没有横幅都会截图 + ML Kit OCR，没有新增固定 delay，但每次开搜多等约 1–3 秒（首次更久）。

### 最小修改

- `prepareSearchLaunchContext` 恢复为只读无障碍节点；横幅仍用节点检测，漏进群聊仍靠浮层 BACK。
...
## [未发布] 2026-08-28 —— 启动期顶部 IM/群消息横幅只等不点

### 验证前记录

- 现象：刚进抖音首页时顶部弹出群/私信横幅，盖住搜索和顶部 Tab；点搜索会点进群，随后 `WAITING_FOR_SEARCH_ENTRY` 超时「Search input did not appear after opening search」。
- 复现：冷启动抖音，等顶部白条（群名 + 预览，右侧可能有「回复」）出现后再开 B 端任务。
- 预期：点搜索前仍确认是首页；横幅在则只等消失，不点「回复」、不点横幅、不点坐标兜底；若已经进群则有界 BACK 回首页，不要当搜索超时停掉。
- 根因假设：`TransientOverlayDetector` 只认直播横幅和开屏广告；开搜三条路径都会点到横幅覆盖的右上角。

### 最小修改

- 检测：顶部约 18% 内的「回复」，或盖住搜索区的宽短可点横条（屏高比例，不用 px）。节点优先，不为此新开 OCR。
- 动作：与开屏广告一样只等不点；`openSearch` 在语义/结构/坐标三条路径前拦下。
- 漏检：`WAITING_FOR_SEARCH_ENTRY` 若已是聊天页，走已有 `recoverInitialSurface`。

### 验证

- 编译：`:app:compileDebugKotlin` 通过。
- 单测：`TransientOverlayDetectorTest`、`SearchFlowRouterTest`、`SearchFlowTest`、`RecoveryFlowRouterTest`、`RecoveryFlowTest`、`InitialHomeSurfacePolicyTest` 通过。
- 真机：未装机。需冷启动抖音、等顶部横幅出现后再开 B 端任务。
- 无新增 px / 点击坐标；横幅高度用屏高比例。
...
## [未发布] 2026-08-28 —— 群聊浮层 BACK，点搜索前补一次顶栏 OCR

### 验证前记录

- 现象：横幅出现后仍点搜索，进入盖在首页上的群聊半屏（X、「发送消息」、打招呼/比心），随后任务停止。
- 预期：横幅在则只等；若已进群聊浮层，有界 BACK 回首页，不要空等搜索框。
- 根因假设：横幅「回复」常无无障碍文本，节点几何又要求可点容器，漏检后仍点搜索；浮层不是 `DIRECT_MESSAGE`（背景 Tab 还在、没有「发送」按钮），后验不恢复。

### 最小修改

- 点搜索前若节点没认出横幅，只做一次顶栏 OCR（`HOME_SEARCH_CHROME`）找「回复」。
- 新增 `GroupChatOverlayDetector`（底部「发送消息」+ 打招呼/比心/@群聊AI）；启动和 `WAITING_FOR_SEARCH_ENTRY` 走已有有界 BACK，不点 X/回复。
- 遮罩未消失时不再用旧 context 点穿。

### 验证

- 编译：`:app:compileDebugKotlin` 通过。
- 单测：`GroupChatOverlayDetectorTest`、`NestedLaunchSurfacePolicyTest`、横幅/搜索路由测试通过。
- 真机：未装机。
- 无新增 px / 点击坐标。

### 下一步

- 真机确认：横幅应 `im_banner_waiting`；若已进群应 BACK 回首页再搜。

---

## [未发布] 2026-08-27 —— 我的页菜单高度与营销内容精简

### 验证前记录

- 现象：我的页菜单行高度不一致；自动化服务与悬浮窗各占一行且带图标；营销内容有「内容 1」标签，随机发送带卡片底和说明。
- 预期：菜单行等高；两条权限提示并排、无图标，「悬浮窗」「开启」；营销内容去掉编号标签，随机发送只留开关和文案。

### 最小修改

- 菜单行固定 `AutomationSizing.Tile` 高度。
- 权限提示并排，去掉图标，文案改为「悬浮窗」「开启」。
- 营销内容去掉「内容 N」；随机发送不再包卡片、不再显示说明。

### 验证

- 编译：`:app:assembleDebug` 通过。
- 真机 OnePlus NE2210 `b33aa309`：已安装并拉起。
- 无新增 px / 点击坐标。
...
## [未发布] 2026-08-27 —— 待办列表微调，我的页与营销内容改造

### 验证前记录

- 现象：待办卡片仍带「B端私信 / 评论私信」标签；勾选框贴在标题行顶部；空态贴着分段页签；「我的」和「营销内容编辑」仍是旧圆角与旧控件。
- 预期：待办行去掉类型标签，勾选框在卡片右侧垂直居中；空态与页签拉开距离；我的页和营销内容页使用现有组件与 token。

### 最小修改

- 待办：去掉 `AppHistoryRow` 类型 badge；`trailing` 改为整卡垂直居中；空态增加 Hero 上边距。
- 我的：账号色块、`AppNoticeRow`、菜单 `AppRecordLine`、退出登录用错误色。
- 营销内容：`AppSegmentedTab`、多行 `AppTextField`、`AppRadio`、`AppSwitchRow`、底部 `AppPrimaryButton`。保存与同步逻辑未改。
...
## [未发布] 2026-08-27 —— 待办与新建任务表单按 pic1 改造

### 验证前记录

- 现象：首页「功能」「今日待办」贴着统计卡；待办页和新建任务表单仍是旧 Tab / 描边输入。
- 预期：两个首页标题加粗并拉开上边距；待办用分段页签、卡片列表、底部主按钮；B 端 / 评论表单用标签输入、选择芯片、开关和底部双按钮。

### 最小修改

- 首页：`AppSectionTitle` 加粗；统计卡下方增加 Hero/Card 间距。
- 待办：`AppSegmentedTab` + `AppHistoryRow` 勾选 + `AppEmptyState` + `AppPrimaryButton`。
- 新建 B 端 / 评论：`AppLabeledField`、`AppChoiceChip`、`AppSwitchRow`、`AppButtonRow`。逻辑未改。

### 验证

- 编译：`:app:assembleDebug` 通过。
- 真机 OnePlus NE2210 `b33aa309`：首页标题加粗并拉开间距；待办为分段页签+卡片+开始任务；B 端 / 评论表单为标签输入、芯片与底部双按钮。
- 无新增 px / 点击坐标。
...
## [未发布] 2026-08-27 —— 首页色块回归与登录页改造

### 验证前记录

- 现象：首页顶部改成了白底摘要，功能入口变成小磁贴；登录页仍是浅灰表单。
- 预期：顶部主蓝色块、底部两侧圆角、统计卡覆盖色块；功能入口为 4:3 横向色块（标题+副标题+图标）；登录页按 app1 问候 → 输入 → 主按钮。

### 最小修改

- 新增 `AppPageHero`、`AppColorBlock`、登录画布字段/按钮。
- 首页顶部色块底部圆角并让统计卡覆盖一部分；B 端 / 评论入口改回色块，评论用同色相的深蓝。
- 登录页使用 `AppLoginCanvas`：问候、半透明输入、白色胶囊登录按钮。未接入免密/社交/注册。

### 验证

- 编译：`:app:assembleDebug` 通过。
- 真机 OnePlus NE2210 `b33aa309`：首页顶部主蓝色块、底部圆角、统计卡覆盖；功能入口为横向色块（标题+副标题+图标）。登录页需退出后查看。
- 无新增 px / 点击坐标。

### 下一步

- 待办页、我的页、新建任务表单按组件继续替换。

---

## [未发布] 2026-08-27 —— 记录进底栏，首页改组件

### 验证前记录

- 现象：任务详情上下空隙偏大，顶部有图标；任务记录藏在「我的」里；首页仍是旧色块卡片。
- 预期：详情更紧、去掉顶部图标；底栏增加「记录」；首页用现有组件重排。

### 最小修改

- 详情：`AppDetailHero` 不再传图标；键值行与分组分隔改为 Tight/Compact。
- 「任务记录」从「我的」菜单挪到底栏，名称「记录」，作为根 Tab。
- 首页：问候 + `AppHeroStats` + `AppIconTile` 功能入口 + 今日待办（空态 `AppEmptyState`，有数据用 `AppHistoryRow`）。去掉评论入口红底。

### 验证

- 编译：`:app:assembleDebug` 通过。
- 真机 OnePlus NE2210 `b33aa309`：已安装并拉起首页。
- 无新增 px / 点击坐标。

### 下一步

- 待办页、我的页、登录页、新建任务表单按组件继续替换。

---

## [未发布] 2026-08-27 —— UI 组件库与设计 token

### 验证前记录

- 现象：页面改造缺少统一组件，色值和圆角只能逐页口述，风格容易发散。
- 预期：截图沉淀成 token + 共享组件；业务页只选组件。主色一条蓝，其余用变淡处理。

### 最小修改

- 主色改为 pic2「提交」`#585EE8`，其余 token 按该色变淡。
- 默认密度收紧：控件约 40.dp，正文字号 13–14.sp，Regular 字重，间距缩小。
- 补空状态（app2）、详情键值（app4）、下划线记录行（app5）。
- 「我的 → 组件预览」可对照参考图验收。任务记录列表已改用筛选胶囊和三行卡片；任务详情改用 app4 键值与 app5 下划线结果行，并移除「复用此任务配置」。
...
- 确认预览页后，再按组件替换登录页（结构用 app1）和任务历史列表（布局用 pic4）。

---

## [未发布] 2026-08-27 —— 退出后登录按钮可点

### 验证前记录

- 现象：退出登录后停留在登录页，账号密码已填，登录按钮点不了。
- 复现：OnePlus NE2210 `b33aa309`，登录页提示「当前安装包尚未配置服务地址」，按钮 `clickable=false`。
- 预期：账号密码填好后可以登录。构建未注入地址时允许填写 HTTPS 服务地址；退出后记住上次成功的地址。

### 最小修改

- 新增 `LoginEndpointPolicy`：构建地址 → 上次成功地址 → 页面填写。
- 退出只清 token，不清上次 HTTPS 地址。
- 构建未注入地址时显示「服务地址」输入框，填好 https 后启用登录按钮。

### 验证

- 单测：`LoginEndpointPolicyTest`、`AuthModelsTest` 通过。
- 真机 OnePlus NE2210 `b33aa309`：未注入构建地址时出现「服务地址」。填入 https 地址、账号、密码后登录按钮可点，点击后发起请求（无效地址返回无法解析主机，证明按钮已启用）。
- 无新增 px / 点击坐标。

### 下一步

- 受管包仍应通过 `-PautomationApiEndpoint` 注入地址，登录页可不显示该字段。

---

## [未发布] 2026-08-27 —— 待办在任务开始后立即移除

### 验证前记录

- 现象：待办页在任务完成后仍保留同一配置。红木沙发-2026？已堆积 3 条。
- 复现：B 端表单「立即开始」多次跑完后打开待办。
- 预期：任务一开始就离开待办；已跑过的条目不再显示。显式「保存」且尚未开始的草稿仍保留。

### 最小修改

- `beginTask` 不再把正在跑的任务写回待办，改为按任务 id 软删除。
- 待办批量开始时同步移除所选条目。
- 启动时清掉已在任务历史中的待办 id。
- 待办列表随 `savedTaskListRevision` 刷新。

### 验证

- 单测：`:app:testDebugUnitTest` 通过。
- 真机 OnePlus NE2210 `b33aa309`：安装后待办 B 端不再出现「红木沙发-2026？」堆积项（0 条）。仍保留未开始的「保存」草稿（室内设计师复用、评论 designer）。
- 无新增 px / 点击坐标。

### 下一步

- 「保存」且尚未开始的草稿继续留在待办；从待办点开始后应立即消失。

---

## [未发布] 2026-08-27 —— 失败用户不计入用户数，并拒绝认证文案当昵称

### 验证前记录

- 现象 1：店铺无私信失败后，任务记录帐号名为「抖音组织认证:」，不是店铺名。
- 现象 2：失败用户仍占用「用户上限」，任务在未凑够私信成功数时结束。
- 现象 3：表单/列表/历史仍写「用户上限」，与待办卡「用户数」不一致。
- 复现：OnePlus NE2210 `b33aa309`，红木沙发-2026？，失败卡 08-27 16:16:10，页面「用户主页」。
- 预期：失败记录保存店铺名；失败/屏蔽/跳过不占用户数，顺延后续用户直到私信成功数达标；界面统一为「用户数」= 私信成功用户。

### 最小修改

- 列表/主页昵称拒绝认证 chrome 与冒号标签；会话页不在主页上覆盖昵称；主页 OCR 在列表名是 chrome 时仍可取证。
- 「用户数」只计 `BLANK_PROBE_VERIFIED` / `MESSAGE_SENT`；失败后继续选后续用户。
- 新建表单、任务记录、任务列表、任务历史文案「用户上限」改为「用户数」。API 字段 `max_users` 不变。

### 验证

- 单测：`:app:testDebugUnitTest` 通过。
- 真机 OnePlus NE2210 `b33aa309`，红木茶桌 / 5 / 空探针：`COMPLETED`，`用户处理结果（7）` = 5 次 `BLANK_PROBE_VERIFIED` + 1 次店铺 `MESSAGE_SEND_FAILED` + 1 次身份不可用。失败后继续选后续用户，直到 5 次私信成功才 `task_completed_user_limit`。
- 失败店铺记录名为「梧空室界木作旗舰店」（OCR 近形字，不是「抖音组织认证:」/「8万」）。
- 新建表单、任务历史、任务详情均显示「用户数」。历史卡 `用户数 5/5 · 失败 2`。
- 无新增 px / 点击坐标。

### 下一步

- 旧任务 16:16 的错误昵称是当时落盘结果，不会改写。新任务已按店铺名记录。

---

## [未发布] 2026-08-27 —— 店铺无私信后退回当前页续接，并拒绝粉丝数当昵称

### 验证前记录

- 现象 1：红木沙发任务进入列表第 3 个纯店铺账号，主页无「私信」只有「联系客服」。退回后翻页，没有接着第 4 个可见用户。
- 现象 2：任务记录里出现帐号名「8万」，对应店铺行粉丝「16.8万」的 OCR 碎片。
- 复现：OnePlus NE2210 `b33aa309`，手动 B 端任务；日志 `MESSAGE_SEND_FAILED` 后立刻 `message_failure_skip` 翻页。
- 预期：退回后继续当前列表第 4 行；记录保存店铺名而非粉丝数。

### 最小修改

- 私信失败/不可用退回列表后，复用空探针的「轮询下一结构行 → 当前页 OCR 续接 → 最后才翻页」。
- 身份与主页/会话昵称拒绝 `8万`、`16.8万` 这类计数碎片。

### 验证

- 单测：`:app:testDebugUnitTest` 通过。
- 真机 OnePlus NE2210 `b33aa309`，红木茶桌 / 5：店铺无私信后 `message_failure_next_visible_poll` → `message_failure_next_visible_requested [next_anchor_top=1185]`，无 `message_failure_skip` 翻页；随后继续当前页第 4、第 5 个用户，`COMPLETED`，失败 1。
- 「8万」未再出现。同一次店铺行曾落到「抖音组织认证：」，已同样排除为装饰文案。
- 无新增 px / 点击坐标。

### 下一步

- 店铺行在有 OCR 店名时应保存店名；若再出现装饰文案当昵称，再收紧 chrome 列表。

---

## [未发布] 2026-08-27 —— B 端空树用右上角条带证明后点已有搜索点

### 验证前记录

- 现象：D 段启动到点搜索约 11.3s。无障碍树空/截断时不 OCR，等节点后再全屏证明 HOME，最后仍点已有 `(0.92, 0.07)`。
- 复现：OnePlus NE2210 `b33aa309`，B 端「红木沙发 / 5 / 无屏蔽词 / 空文本」。
- 预期：空树阶段对右上角条带取证，无开屏广告则点现有归一化搜索点；不新增点击坐标，不误点广告/直播/相机。
- 假设：树缺失时像素上已有 HOME 顶栏；OCR `0.50–1.0 × 0–0.20` 条带看到「推荐 / 关注 / 搜索」即可证明。

### 最小修改

- 新增 `EmptyTreeHomeSearchChromePolicy` 与 `OcrRegion.HOME_SEARCH_CHROME`（屏幕比例，非 px）。
- B 端 `WAITING_FOR_HOME` 在无搜索节点时 OCR 该条带；证明 HOME 后仍走节点 → 结构 → `searchEntryNormalizedFallback`。
- 评论任务不走此路径。开屏广告检测仍为等待、不点击。
- 无新增点击坐标。

### 验证

- 单测：`:app:testDebugUnitTest` 通过。
- 真机 OnePlus NE2210 `b33aa309`：红木沙发 / 5 / 无屏蔽词 / 空探针，`COMPLETED`，5 次 `BLANK_PROBE_VERIFIED`。
- 总时长 89.7s（上一轮 95.5s）。D 启动到点搜索 **8.8s**（上一轮 11.3s）。
- 开屏条带 `chrome_hits=0` 未点；截断树后 `chrome_hits=2` 证明 HOME，随后 `search_entry_selector_fallback` 点已有 `(0.92, 0.07)`。无开屏广告点击、无直播/相机误触。
- 无新增像素常量；条带为屏幕比例；点击坐标未新增。

### 下一步

- 开屏首帧条带未命中后，会短时复用该 OCR，HOME 顶栏出现后才第二次取证。若还要压 D，可在未命中时不要把开屏 OCR 当后续证据。

---

## [未发布] 2026-08-27 —— B 端页面等待按评论模块思路收紧

### 验证前记录

- 现象：红木沙发 / 5 人 / 空探针约 103.7s。回列表后结构行未就绪就走 OCR 双样本；HOME 已认出仍等第二帧全屏 OCR；点行后 UNKNOWN 拍 page_probe；B 端启动误跑 nested_comment_surface。
- 复现：OnePlus NE2210 `b33aa309`，B 端「红木沙发 / 5 / 无屏蔽词 / 空文本」。
- 预期：同一路径更短，仍 5 次 `BLANK_PROBE_VERIFIED`，不翻页、不改点击安全门。

### 最小修改

- 回列表后最多再轮询 5 次结构下一行，失败才 OCR。
- B 端已分类的 HOME/搜索页不再等第二帧 OCR；评论启动仍要两帧。
- `WAITING_FOR_PROFILE` / 打开私信过渡的 UNKNOWN 跳过全屏 page_probe，会话入口 OCR 仍保留。
- B 端启动不跑 `nested_comment_surface` OCR。
- 私信页已确认后的后验等待 900ms→250ms。
- 首轮真机：BACK 首采样 120ms 和私信入口后验 180ms 会打到旧页，已回退为 200ms / 500ms。
- 无新增 px / 坐标常量。
...
- 单测：`:app:testDebugUnitTest` 通过。
- 真机 OnePlus NE2210 `b33aa309`：红木沙发 / 5 / 无屏蔽词 / 空探针，`COMPLETED`，5 次 `BLANK_PROBE_VERIFIED`。
- 总时长 95.5s（基线 103.7s，约 −8s）。C 回列表续接 6.2–9.6s（基线 10.3–10.8s），未再走 OCR 双样本。D 启动到点搜索 11.3s（基线 14.9s），`ocr_page_stable_skipped HOME`，无 `nested_comment_surface`。
- 无新增像素常量。dp 与 0–1 坐标未混用。

### 下一步

- E（搜用户 Tab）本轮未改，受结果页加载波动。
- 列表身份 OCR、树截断仍在。
...
- 现象：首页只展示 3 条待办，标题右侧固定显示「查看全部」；待办卡片使用蓝色装饰条，字段层级不清晰，不能删除。
- 预期：首页按最新任务展示最多 5 条，超过时底部提供「查看全部」；待办页统一显示低优先级标题、字段标签和值，并支持左划确认删除。

### 最小修改

- 本地待办保存更新时间，读取时按最新时间倒序；首页最多显示 5 条，超过后显示居中的「查看全部」链接。
- 待办行改为全宽白底、内边距和淡色分隔线；移除蓝色竖条。标题可换行，搜索词、用户数、屏蔽词、地区仅在有值时展示，并突出字段值。
- 左划只展开固定宽度的红色「删除」按钮；点击按钮后才弹出确认框。确认操作写入 `deleted=true`，记录保留在本地持久化数据中但不再显示。
- 待办列表移除「待办任务」标题；多选框改为 16dp 直角细边框，字段行设置 24dp 最小高度并垂直居中；左划删除按钮固定为 88dp 宽。
- 首页今日待办复用待办的标题、字段和多选布局，但不支持左划删除；B 端任务改用极浅蓝底，评论私信任务使用极浅黄底。
- 首页 B 端待办背景恢复为原有淡蓝色；待办、总览和功能入口卡片统一为约 6dp 的小圆角，待办多选框使用更浅的蓝灰色边框和浅蓝选中态。
- 修复左划时整行内容被移出屏幕：任务内容保持原位，右侧仅覆盖展开 88dp 的删除按钮。
- 删除按钮改用父容器尺寸测量，覆盖整条待办行高度；开始任务按钮改为 6dp 圆角。
- 我的页面移除顶部标题，所有有色块和菜单容器统一为 6dp 圆角；所有分隔线统一 `AutomationDivider`，待办和我的菜单最后一项不再显示分隔线。
- 首页今日待办的多选框改为查看眼睛图标；底栏压缩为 64dp，图标/文字缩小，未选中项使用浅灰色，选中项使用蓝色且无图标背景。
- 我的页面移除授权状态提示块，新增自动化服务和悬浮窗功能状态入口；未开启时显示「点击开启」并跳转到对应的 Android 系统设置页。
- 底栏将图标和文字合并为紧凑布局，图标与文字之间仅保留 1dp 间距；「点击开启」按钮取消默认最小高度，使自动化服务状态块与悬浮窗功能状态块等高。

### 验证

- 单测：`TaskDomainTest` 覆盖 B 端和评论私信待办字段的显示顺序与条件展示，通过。
- **真机**：OnePlus NE2210 / `b33aa309`。待办两类标签页均显示新字段样式；移除列表标题，字段标签和值的垂直中心一致；左划只显示 88dp 删除按钮且任务标题和字段仍可见，删除按钮覆盖整条待办行高度，点击后才弹出确认框，取消后原有待办仍保留。底栏图文间距已收紧；我的页面已无顶部标题、已移除授权状态提示块，自动化服务与悬浮窗功能状态块均为 42dp 高，自动化服务未开启时点击「点击开启」已进入系统无障碍设置。未确认删除设备上的真实待办，避免改变现有数据。

### 几何 / dp 检查

- 新增间距均使用 `dp`；未新增屏幕像素判断或点击坐标。

### 下一步

- 如需验证删除确认后的隐藏结果，可在不含生产待办的测试设备上创建临时任务后确认删除。

---

## [未发布] 2026-08-26 —— App 任务发布同步到后台

### 验证前记录

- 现象：App 本地保存/开始的任务只在手机待办里，后台看不到。
- 预期：发布（含保存到待办、立即开始、排队开始）后后台任务管理可见；运行中只能取消不能删除；后台仍可创建并下发任务。
- 本轮：按已确认的三点实现同步链路，不改无障碍点击策略。

### 最小修改

- 保存到待办、开始本地任务时，异步 `POST /automation/mobile/tasks`（`local_task_id` 幂等），不上传真实私信正文。
- 发布成功后绑定 `remoteTaskId`，后续暂停/完成/记录仍走原有写回队列。
- 后台任务表增加来源、类型、本地任务 ID、账号、设备摘要、评论配置快照；管理端增加任务管理页。

### 真机验证

- **设备**：OnePlus NE2210 / `b33aa309`。未杀抖音、未开新任务，只做历史回填。
- 本地 86 条历史 + 2 条待办、209 条带身份的用户记录，全部写入后台：App 任务 88，用户记录 209。
- 状态对齐：待办 READY 2、暂停 17、完成 25、失败 42、取消 2。无私信正文。
- 首轮 23 条无记录失败任务卡在 READY（`READY→FAILED` 422）；补包后按 RUNNING 中转重试，23/23 成功。
...
- 新保存/开始任务的实时发布仍建议各跑 1 条确认。
- 已完成的待办再次开始时，远端状态机仍不允许 COMPLETED→RUNNING，可能只在本地重跑。
...
- 移动端退出改为先调用服务端释放当前 License/设备绑定，成功或服务端已拒绝旧 token 后才清空本机 Keystore 密钥；网络异常保留本机会话并在确认框内提示重试。
- “我的”退出确认框明确展示服务端同步语义和进行中状态，避免将本地退出误认为已解除后台绑定。
- 后端新增一账号一有效设备限制、账号设备绑定查询/解绑和移动端退出接口；旧物理设备管理接口保持原语义。

### 已验证项

- Android：`./gradlew testDebugUnitTest lintDebug assembleDebug` 通过。
- 后端：`backend/tests/test_api_module_automation.py` 为 `7 passed`，覆盖同设备轮换、跨设备拒绝、后台解绑、退出失效。
- **真机 OnePlus NE2210 / b33aa309 / Android 16**：账号密码登录、规则目录读取、heartbeat、我的页退出和后台绑定释放闭环通过；未打开抖音或执行任务，结束时 App 停在登录页。

### 几何与安全检查

- 未新增固定 px、自动化坐标、OCR、图像匹配或手势时长；继续仅持久化应用作用域设备标识的 SHA-256 摘要。

### 交接记录

- [`2026-08-25-account-device-logout-sync.md`](docs/2026-08-25-account-device-logout-sync.md)

---

## [未发布] 2026-08-25 —— b33aa309 真机非执行型 E2E 冒烟

### 已验证项

- 覆盖安装当前 Debug APK 后，OnePlus NE2210（`b33aa309` / Android 16）冷启动可恢复移动端授权会话。
- 验证“首页 → 我的 → 账号与设备 → heartbeat → 返回我的 → 任务记录 → 返回我的 → 待办 → 首页 → B 端私信 → 返回首页 → 评论私信 → 返回首页”路径。
- B 端搜索词校验、评论匹配模式切换、任务记录/待办空态、系统返回和“我的”服务状态均正常；未保存、挂起或启动任务，未打开抖音或发送消息。
- 测试结束时 App 已停止并返回系统桌面。

### 几何与安全检查

- 本阶段不修改 Android 业务或界面代码；未新增固定 px、自动化坐标、OCR/图像匹配或手势时长。

### 交接记录

- [`2026-08-25-b33aa309-mobile-e2e-smoke.md`](docs/2026-08-25-b33aa309-mobile-e2e-smoke.md)

---

## [未发布] 2026-08-25 —— 任务表单横向布局与入口收敛

### 修改内容

- B 端任务页标题改为“新建 B 端获客任务”，删除基础信息、搜索条件、筛选条件、执行限制四个页内大标题与顶部服务状态；字段使用左标签、右输入框的紧凑行式布局。
- 评论任务页标题改为“新建评论区私信任务”，固定为搜索指定用户入口，隐藏开始位置及其两个选项，移除任务配置标题和当前用户主页专用挂起操作。
- 表单选择项统一为 primary 蓝色双态：未选中白底蓝字蓝框，选中蓝底白字；保存/立即开始按钮分别使用白底蓝字蓝框、蓝底白字。
- 自动化服务状态统一移动到“我的”页展示；任务草稿、校验、保存、启动、无障碍、抖音识别和网络逻辑保持不变。

### 已验证项

- `git diff --check`、`./gradlew :app:testDebugUnitTest :app:lintDebug :app:assembleDebug -PautomationApiEndpoint=<受管 HTTPS 服务地址>` 通过。
- **真机 OnePlus NE2210 / b33aa309 / Android 16**：验证 B 端表单、评论表单和“我的”页的标题、字段布局、蓝色选中/未选中态与服务状态。测试中只切换表单选择项，未保存、挂起或启动任务，未打开抖音；结束后 App 已停止并返回系统桌面。

### 几何与兼容性检查

- 新增/变更尺寸均为 Compose `dp`（50dp 字段、46dp 按钮、34dp 选择项、72dp 最小选择宽、80–88dp 标签宽、8dp/10dp 圆角、1dp 描边）；未新增固定 px 或自动化几何逻辑。

### 交接记录

- [`2026-08-25-task-form-horizontal-layout.md`](docs/2026-08-25-task-form-horizontal-layout.md)
...
- `./gradlew :app:assembleRelease -PautomationApiEndpoint=<受管 HTTPS 服务地址>` 通过；当前仅产生未签名 Release APK，未安装或发布。

### 已记录阻塞项

- 后端“一账号一设备、后台解绑”与后端未提交移动端登录基线重叠，继续保持隔离，避免覆盖或混入未知改动。
- Release 发布仍需 Android 签名密钥和发布授权。

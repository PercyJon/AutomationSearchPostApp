...
1. **稳定可见的抖音首页仍可能被判为 `UNKNOWN`（历史 17:12 签名）。** 2026-08-25 真机诊断：系统调起弹窗期间为 `MISSING_NODES`；弹窗消失后稳定首页 `nodes=213` 被既有语义搜索候选判为 `HOME` 并进入搜索。17:12 的 `OCR_NOT_TRIGGERED`（`nodes=71`）本轮未复现，因此未实施有界首页 OCR。
...
1. **仅当 17:12 类稳定首页 UNKNOWN 再出现时，才做评论任务有界 OCR。**
   - 前置：`waiting_for_home_unknown [cause=OCR_NOT_TRIGGERED]` 且首页已稳定可见。
   - 动作：仅对评论任务 `WAITING_FOR_HOME` 的 `UNKNOWN` 增加顶部/底部导航带 OCR 双帧确认；OCR 只能归类为 `HOME`，搜索入口仍走既有节点顺序。
   - 验收：稳定首页可进入搜索；不得放宽全局 PageDetector，不得裸坐标点击。

2. **仅当节点、评论模板和 OCR 均无法形成评论入口时，再验收双锚点实点击。**
